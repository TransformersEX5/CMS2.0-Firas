...
namespace app\modules\programfeedocument\controllers;
...
use app\modules\programfeedocument\models\tbluploaddocument;

/**
 * Default controller for the `programfeedocument` module
 */
class DefaultController extends Controller
{
    ...
    {
        if (Yii::$app->user->can('Program-Fee-Document-Index')) {
    ...
    {
        if (Yii::$app->user->can('Program-Fee-Document-Upload')) {
    ...
    public function actionUpload()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && Yii::$app->request->isAjax) {
            ...
            if ($model->save()) {
                ...
                $uploadPath = Yii::getAlias('@documentupload/programfeedocument/' . $model->Year);
    ...
    public function actionDocumentlist()
    {
        ...
        CONCAT(tbluser.FullName, '<br>',DATE_FORMAT(tbluploaddocument.TransactionDate, '%d/%m/%Y %H:%i:%s')) AS UploadBy, tbluploaddocument.Remarks,
        tbluploaddocument.StatusId, tblstatusai.Status, Visibility
        ...
        WHERE (UploadDocument REGEXP '$searchbox' OR YEAR REGEXP '$searchbox') AND UploadDocumentTypeId = 4 AND Visibility = 1
