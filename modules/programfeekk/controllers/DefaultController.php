...
namespace app\modules\programfeekk\controllers;


use Yii;
use app\modules\programfeekk\models\Tblprogramfeecategory;
use app\modules\programfeekk\models\Tblprogramfeecategorydetail;
use app\modules\programfeekk\models\Tblprogramfeecategorystatus;
use app\modules\programfeekk\models\Tblprogramfee;
        ...
        if (yii::$app->user->can('ProgramFee-Create')) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->save(true)) {
                    ...
                    $command = \Yii::$app->dbcitykk->createCommand('UPDATE tblprogramfeecategorystatus SET CurrentStatusId = 0  WHERE ProgFeeCatId = ' . $model->ProgFeeCatId);
                    $command->execute();
                    ...
                    $command = \Yii::$app->dbcitykk->createCommand(' UPDATE tblprogramfeecategory SET ProgFeeCode = "' . $new_code . '" WHERE ProgFeeCatId = ' . $model->ProgFeeCatId);
                    $command->execute();


                    Yii::$app->session->setFlash('success', "Record successfully create.");

                    return $this->redirect(Yii::$app->homeUrl . 'programfeekk');
                } else {
        ...
        if (Yii::$app->user->can('ProgramFee-Edit')) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->save(true)) {
                    ...
                    return $this->redirect(Url::base() . '/programfeekk/default/index');
                } else {
