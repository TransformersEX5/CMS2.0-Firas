            ...
            <div class="card">
                <div class="card-header align-items-center d-flex text-white bg-warning   ">
                    ...
                    <div class="flex-shrink-0">
                        ...
                        <button id="New" class="showModal_New_Joiner btn btn-success btn-sm" type="button"  > New Joiner <i
                        class="icon-file"></i></button>
                            ...
                            <button id="View" class="showModal_View btn btn-success btn-sm" type="button"> View <i
                                    class="icon-file"></i></button>
                            <button id="Permission" class="showModal_Permission btn btn-success btn-sm" type="button"> Permission <i
                                    class="icon-file"></i></button>
                        </div>
                    </div>
                </div><!-- end card header -->
...
<?php
/**********************************************************************************/
$url = Url::toRoute(['/user/userlist']);
$create = Url::toRoute(['/staff/default/create']);
$update = Url::toRoute(['/staff/default/update']);
$view = Url::toRoute(['/staff/default/view']);
$permission = Url::toRoute(['/user/permission']);

$showModal_New_Joiner = Url::toRoute(['/staff/staffdetail/index']);
...
$('#btn_search').click(function () {
      ...
      $('#cmstable1').dataTable().empty();
		...
		var table = $('#cmstable1').DataTable({
			...
			"columns": [
				...
				{"data":"DeptCatDesc"},	
                {"data":"Status"},	   
                {"data":"", title: '#', wrap: true,"defaultContent":'<div class="btn-group showModal_View"> <button type="button" id="del-btn" value="0" class="btn btn-warning btn-sm" >View</button></div>'+" " +'<div class="btn-group showModal_Update"> <button type="button" id="del-btn" value="0" class="btn btn-warning btn-sm" >Edit</button></div>'},
  ...
  $(document).on('click', '.showModal_New_Joiner', function () {
      alert("cc");

    window.location.href = '$showModal_New_Joiner';
...
$(document).on('click', '.showModal_New', function () {
      ...
      $.ajax({
            ...
            {     
				...
				document.getElementById('modalHeader-xl').innerHTML = '<h4>New Staff</h4>';    
	...
	$(document).on('click', '.showModal_View', function () {
      ...
      $.ajax({
            ...
            {     
				$('#modal-xl').modal('show');
                $('#modalContent-xl').html(response).modal();							
				document.getElementById('modalHeader-xl').innerHTML = '<h4>View Staff</h4>';
	...
	$(document).on('click', '.showModal_Update', function () {
      ...
      $.ajax({
            ...
            {                     
                $('#modal-xl').modal('show');
                $('#modalContent-xl').html(response).modal();							
				document.getElementById('modalHeader-xl').innerHTML = '<h4>Edit Staff</h4>';
            },
            error  : function (e) {
				   // console.log(e);
            }
  ...
  /**********************************************************************************/
	...
	$(document).on('click', '.showModal_Permission', function () {
    ...
    $.ajax({
          ...
          {                     
              $('#modal-xl').modal('show');
              $('#modalContent-xl').html(response).modal();							
              document.getElementById('modalHeader-xl').innerHTML = '<h4>Set Permission</h4>';
          },
          error  : function (e) {
                 // console.log(e);
          }
