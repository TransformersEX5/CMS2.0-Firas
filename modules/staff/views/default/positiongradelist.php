...
$this->title = Yii::t('app', 'Position Grade List ');
...
<?php
/**********************************************************************************/
$url = Url::toRoute(['/staff/default/getpositiongrade']);
$create = Url::toRoute(['/staff/default/positiongradenew']);
...
$(document).ready(function () {
    ...
    $('#btn_search').click(function () {
        ...
        var table = $('#cmstable1').DataTable({
            ...
            "columns": [
                {
                    "data": "PositionGradeId", "width": '0.1%', class:"text-center text-dark",
                    "render": function (data, type, row, meta) {
                    return meta.row + meta.settings._iDisplayStart + 1;
                    }
                },	
                {"data":"PositionGrade", "width": '10.0%', class:"text-dark"},	
                {"data":"PositionDesc", "width": '10.0%', class:"text-dark"},										
            ]
        });
        ...
        $('#cmstable1 tbody').on('click', 'tr', function() {
			...
			var x = d['PositionGrade'];
			var y = d['PositionGradeId'];
...
$(document).on('click', '.showModal_New', function () {
      ...
      $.ajax({
            ...
            {     
				...
				document.getElementById('modalHeader-xs').innerHTML = '<h4>New Position Grade</h4>';    
  ...
  /**********************************************************************************/
	...
	$(document).on('click', '.showModal_Update', function () {
      var posgradeId = document.getElementById('select_id').value;
      if(posgradeId != '')
      {
        var form = $(this);
        $.ajax({
	    	url: '$create?posgradeId='+posgradeId,
            type   : 'GET',
			data: {
            ...
            {                     
                $('#modal-xs').modal('show');
                $('#modalContent-xs').html(response).modal();							
				document.getElementById('modalHeader-xs').innerHTML = '<h4>Edit Position Grade</h4>';
            },
            error  : function (e) {
      ...
      {
        alert('Please select a position grade.');
        return false;  
      }
