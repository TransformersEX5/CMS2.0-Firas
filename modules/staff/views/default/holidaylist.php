...
$this->title = Yii::t('app', 'Holiday List ');
...
<?php
/**********************************************************************************/
$url = Url::toRoute(['/staff/default/getholiday']);
$create = Url::toRoute(['/staff/default/holidaynew']);
...
$(document).ready(function () {
    ...
    $('#btn_search').click(function () {
        ...
        var table = $('#cmstable1').DataTable({
            ...
            "columns": [
                {
                    "data": "HolidayId", "width": '0.1%', class:"text-center text-dark",
                    "render": function (data, type, row, meta) {
                    return meta.row + meta.settings._iDisplayStart + 1;
                    }
                },	
                {"data":"Holiday", "width": '10.0%', class:"text-dark"},	
                {"data":"HolidayStatus", "width": '10.0%', class:"text-dark"},										
            ]
        });
...
$(document).on('click', '.showModal_New', function () {
      ...
      $.ajax({
            ...
            {     
				...
				document.getElementById('modalHeader-xs').innerHTML = '<h4>New Holiday</h4>';    
  ...
  /**********************************************************************************/
	...
	$(document).on('click', '.showModal_Update', function () {
      var holidayId = document.getElementById('select_id').value;
      if(holidayId != '')
      {
        var form = $(this);
        $.ajax({
	    	url: '$create?holidayId='+holidayId,
            type   : 'GET',
			data: {
            ...
            {                     
                $('#modal-xs').modal('show');
                $('#modalContent-xs').html(response).modal();							
				document.getElementById('modalHeader-xs').innerHTML = '<h4>Edit Holiday</h4>';
            },
            error  : function (e) {
