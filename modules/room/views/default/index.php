...
$this->title = Yii::t('app', 'Room List');
...
<div class="tblprogram-index">
    ...
    <div class="row">
                    ...
                    <div class="row g-3">
                        <div class="col-xl-6">
                            <div class="search-box">
                                <input type="text" class="form-control search" id="txtSearch" placeholder="Search for room code, room name, room type...">
                        ...
                        <div class="col-xl-6">
                            <div class="row g-3">
                                ...
                                <!--end col-->
                                <div class="col-sm-4">
                                    <?= Html::dropDownList('RoomTypeId', null, Yii::$app->common->getRoomType(), [
                                        'prompt' => '- Room Type -',
                                        'class' => 'form-select mb-3',
                                        'id' => 'txtRoomTypeId'
                                    ]) ?>
        ...
        <?php
        /**********************************************************************************/
        $url = Url::toRoute(['/room/default/roomlist']);
        $create = Url::toRoute(['/room/default/create']);
        $update = Url::toRoute(['/room/default/update']);
        $view = Url::toRoute(['/room/default/view']);
...
$('#btn_search').click(function () {
      ...
      $('#cmstable1').dataTable().empty();
		...
		var table = $('#cmstable1').DataTable({
            ...
            pageLength: 12,paging:true,
            scrollY: true,autoWidth: true, 
			dom: 'Bfrtip',                      
			ajax: { 
				url: '$url',
				type: 'GET',
				data: {
                    ...
                    txtRoomTypeId:$("#txtRoomTypeId").val(),                            
					}
			},
...
$(document).on('click', '.showModal_New', function () {
      ...
      $.ajax({
            ...
            type   : 'GET',
			data: {
                //RoomId: $("#select_id").val(),
                 },
             success: function (response) 
            {     
				$("#modal-lg").modal("show");
                $("#modalContent-lg").html(response).modal();							
				document.getElementById('modalHeader-lg').innerHTML = '<h4>New Room</h4>';    
	...
	$(document).on('click', '.showModal_View', function () {
      ...
      $.ajax({
            ...
            {     
				$('#modal-lg').modal('show');
                $('#modalContent-lg').html(response).modal();							
				document.getElementById('modalHeader-lg').innerHTML = '<h4>View Room</h4>';
	...
	$(document).on('click', '.showModal_Update', function () {
      ...
      $.ajax({
            ...
            {                     
                $('#modal-lg').modal('show');
                $('#modalContent-lg').html(response).modal();							
				document.getElementById('modalHeader-lg').innerHTML = '<h4>Edit Room</h4>';
            },
            complete: function() {               
	            var modal = document.getElementById('modal-lg');
                modal.addEventListener('hidden.bs.modal', function (e) {
