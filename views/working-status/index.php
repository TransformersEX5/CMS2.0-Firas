...
$this->title = Yii::t('app', 'Working Status');
$this->params['breadcrumbs'][] = $this->title;
?>

<div class="tblwoekingstatus-index">

    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header align-items-center d-flex text-white bg-success ">
...
<?php
/**********************************************************************************/
$url = Url::toRoute(['/working-status/workingstatuslist']);
$create = Url::toRoute(['/working-status/create']);
$update = Url::toRoute(['/working-status/update']);
$view = Url::toRoute(['/working-status/view']);
...
$('#btn_search').click(function () {
	...
	getButtonDisable();
      ...
      $('#cmstable1').dataTable().empty();
		...
		var table = $('#cmstable1').DataTable({
			...
			"columns": [
				{"data":"WorkingStatusId", "visible": false},
				{"data":"WorkingStatusDesc","width": '38%'},	
                {"data":"WorkingStatusDesc","width": '38%'},	    
	...
	$('#cmstable1 tbody').on('click', 'tr', function() {
     // console.log(table.row(this).data());
			...
			var x = d['WorkingStatusDesc'];
			var y = d['WorkingStatusId'];
...
$(document).on('click', '.showModal_New', function () {
      ...
      $.ajax({
            ...
            {     
				...
				document.getElementById('modalHeader-md').innerHTML = '<h4>Working Status</h4>';    
	...
	$(document).on('click', '.showModal_View', function () {
      ...
      $.ajax({
            ...
            {     
				$('#modal-md').modal('show');
                $('#modalContent-md').html(response).modal();							
				document.getElementById('modalHeader-md').innerHTML = '<h4>View Working Status</h4>';
	...
	$(document).on('click', '.showModal_Update', function () {
      ...
      $.ajax({
            ...
            {                     
                $('#modal-md').modal('show');
                $('#modalContent-md').html(response).modal();							
				document.getElementById('modalHeader-md').innerHTML = '<h4>Edit Working Status</h4>';
            },
            error  : function (e) {
				   // console.log(e);
            }
