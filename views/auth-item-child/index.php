...
$this->title = Yii::t('app', 'Step 2: Set Role vs Permission');
...
<?php
/**********************************************************************************/
$url    = Url::toRoute(['/auth-item-child/authitemchildlist']);
$create = Url::toRoute(['/auth-item-child/create']);
$update = Url::toRoute(['/auth-item-child/update']);
$view   = Url::toRoute(['/auth-item-child/view']);
$delete = Url::toRoute(['/auth-item-child/delete']);
      ...
      $('#cmstable1').dataTable().empty();
		...
		var table = $('#cmstable1').DataTable({
			...
			"columns": [
				...
				{"data":"parent","width": '38%'},	
                {"data":"child","width": '38%'},	                
				// {"data":"Status","width": '10%'},	
                {"data":"", title: 'Action', wrap: true,"defaultContent":'<div class="btn-group showModal_View"> <button type="button" id="add-btn" value="0" class="btn btn-warning btn-sm" >View</button></div>'
                            +" " +'<div class="btn-group showModal_Update"> <button type="button" id="edit-btn" value="0" class="btn btn-warning btn-sm" >Edit</button></div>'
                            +" " +'<div class="btn-group showModal_Delete"> <button type="button" id="del-btn" value="0" class="btn btn-danger btn-sm" >Remove</button></div>'},
      ...
      $.ajax({
            ...
            {    // console.log(response);
             ...
             // console.log(response.message);
				...
				document.getElementById('modalHeader-lg').innerHTML = '<h4>Create Role vs Permission</h4>';    
  ...
  /**********************************************************************************/
	$(document).on('click', '.showModal_View', function () {
      ...
      $.ajax({
            ...
            type   : 'GET',
			data: {
                parent: $("#select_id").val(),
                child: $("#select_data").html(),
                 },
             success: function (response) 
            {     
				$('#modal-lg').modal('show');
                $('#modalContent-lg').html(response).modal();							
				document.getElementById('modalHeader-lg').innerHTML = '<h4>View Role vs Permission</h4>';
	...
	$(document).on('click', '.showModal_Update', function () {
      ...
      $.ajax({
            ...
            {                     
                $('#modal-lg').modal('show');
                $('#modalContent-lg').html(response).modal();							
				document.getElementById('modalHeader-lg').innerHTML = '<h4>Edit Role vs Permission</h4>';
            },
            error  : function (e) {
				   // console.log(e);
            }
